...
help:
	...
	@echo ""
	@echo "Packages:"
	@echo "  make packages-save    Save currently installed packages to repo"
	@echo "  make packages-diff    Show diff between installed and saved packages"
	@echo ""
	@echo "System:"
...
packages-save:
	@echo "Saving current package lists..."
	@mkdir -p packages
	@echo "# Native packages (pacman)" > packages/pacman.txt
	@echo "# Auto-generated by: make packages-save" >> packages/pacman.txt
	@echo "# Date: $$(date +%Y-%m-%d)" >> packages/pacman.txt
	@echo "" >> packages/pacman.txt
	@pacman -Qqen | sort >> packages/pacman.txt
	@echo "" > packages/aur.txt
	@echo "# AUR packages" > packages/aur.txt
	@echo "# Auto-generated by: make packages-save" >> packages/aur.txt
	@echo "# Date: $$(date +%Y-%m-%d)" >> packages/aur.txt
	@echo "" >> packages/aur.txt
	@pacman -Qqem | sort >> packages/aur.txt
	@echo "Saved:"
	@echo "  pacman.txt: $$(pacman -Qqen | wc -l) packages"
	@echo "  aur.txt:    $$(pacman -Qqem | wc -l) packages"
	@echo "Review changes with: git diff packages/"

packages-diff:
	@echo "Package differences (installed vs saved):"
	@echo ""
	@echo "─── Native (pacman) ───"
	@if [ -f packages/pacman.txt ]; then \
		SAVED=$$(grep -v '^#' packages/pacman.txt | grep -v '^$$' | sort); \
		INSTALLED=$$(pacman -Qqen | sort); \
		ADDED=$$(comm -13 <(echo "$$SAVED") <(echo "$$INSTALLED")); \
		REMOVED=$$(comm -23 <(echo "$$SAVED") <(echo "$$INSTALLED")); \
		if [ -n "$$ADDED" ]; then \
			echo "  Added (not in saved list):"; \
			echo "$$ADDED" | sed 's/^/    + /'; \
		fi; \
		if [ -n "$$REMOVED" ]; then \
			echo "  Removed (in saved list but not installed):"; \
			echo "$$REMOVED" | sed 's/^/    - /'; \
		fi; \
		...
		echo "  packages/pacman.txt not found! Run: make packages-save"; \
	fi
	@echo ""
	@echo "─── AUR ───"
	@if [ -f packages/aur.txt ]; then \
		SAVED=$$(grep -v '^#' packages/aur.txt | grep -v '^$$' | sort); \
		INSTALLED=$$(pacman -Qqem | sort); \
		ADDED=$$(comm -13 <(echo "$$SAVED") <(echo "$$INSTALLED")); \
		REMOVED=$$(comm -23 <(echo "$$SAVED") <(echo "$$INSTALLED")); \
		if [ -n "$$ADDED" ]; then \
			echo "  Added (not in saved list):"; \
		...
		echo "  packages/aur.txt not found! Run: make packages-save"; \
	fi
